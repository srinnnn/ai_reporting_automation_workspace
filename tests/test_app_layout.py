...
class AppLayoutTests(unittest.TestCase):
    ...
    def test_dashboard_prioritizes_priority_entry_and_moves_project_stages_to_secondary_page(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertIn("全局筛选", dashboard)
            self.assertIn("<span>项目</span>", dashboard)
            self.assertIn("<span>品牌</span>", dashboard)
            self.assertIn("<span>优先级</span>", dashboard)
            self.assertIn("<span>状态</span>", dashboard)
            self.assertIn('class="selected-brand-banner"', dashboard)
            self.assertIn('href="/workspace?brand=ANTA"', dashboard)
            self.assertIn('action="/"', dashboard)
            self.assertNotIn("巡查", dashboard)
            self.assertIn("当前品牌 Workspace", dashboard)
            self.assertIn("ANTA 安踏 KPI", dashboard)
            self.assertIn("已接项目", dashboard)
            self.assertIn("已开发反馈汇总", dashboard)
            self.assertIn("快捷导航", dashboard)
            self.assertIn("安踏周报/月报", dashboard)
            self.assertIn("安踏即时零售", dashboard)
            self.assertIn("查看报表", dashboard)
            self.assertIn("开发排期", dashboard)
            ...
            self.assertNotIn("隐藏能力", dashboard)
            self.assertNotIn("最近处理记录", dashboard)
            self.assertNotIn("platform-hero", dashboard)
            self.assertNotIn("可提效项目", dashboard)
            ...
            self.assertIn('href="/development-roadmap"', dashboard)
            ...
            bsh_dashboard = app._dashboard(user, "BSH")
            self.assertIn("BSH 博西 KPI", bsh_dashboard)
            self.assertIn('href="/workspace?brand=BSH"', bsh_dashboard)
            self.assertIn("博西短彩信数据处理", bsh_dashboard)
            self.assertIn("博世/西门子短彩信规划复核", bsh_dashboard)
            self.assertNotIn("安踏周报/月报", bsh_dashboard)
            self.assertNotIn("安踏即时零售", bsh_dashboard)
            self.assertNotIn("ECCO活动配置", bsh_dashboard)
