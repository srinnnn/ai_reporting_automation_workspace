...
`homepage.png` is copied from the Product Owner approved design source. `homepage-structure.json` is the operational Playwright comparison contract for the locked homepage structure. `tests/visual/runtime/homepage.png` is the Playwright runtime screenshot from `http://127.0.0.1:8785/`.
